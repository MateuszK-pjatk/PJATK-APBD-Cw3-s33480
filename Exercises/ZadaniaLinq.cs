...
public sealed class ZadaniaLinq
{
    ...
    public IEnumerable<string> Zadanie04_PierwszyPrzedmiotAnalityczny()
    {
        ...
        if (przedmiot != null)
        {
            return new[] { $"{przedmiot.Nazwa} {przedmiot.DataStartu}" };
        }
        else
        {
            return new[] { "Nie znaleziono żadnego przedmiotu w kategorii Analytics." };
        }
    }
    ...
    public IEnumerable<string> Zadanie12_ParyStudentPrzedmiot()
    {
        return DaneUczelni.Zapisy
            .Join(
                ...
                zapis => zapis.StudentId,
                student => student.Id,
                (zapis, student) => new { Zapis = zapis, Student = student })
            .Join(
                DaneUczelni.Przedmioty,
                wynikTymczasowy => wynikTymczasowy.Zapis.PrzedmiotId,
                przedmiot => przedmiot.Id,
                (wynikTymczasowy, przedmiot) =>
                    $"{{wynikTymczasowy.Student.Imie}} {{wynikTymczasowy.Student.Nazwisko}} {{przedmiot.Nazwa}}");
    }
    ...
    public IEnumerable<string> Wyzwanie03_ProwadzacyISredniaOcenNaIchPrzedmiotach()
    {
        return DaneUczelni.Prowadzacy
            .GroupJoin(
                ...
                pr => pr.Id,
                pz => pz.ProwadzacyId,
                (pr, oceny) => $"{pr.Imie} {pr.Nazwisko} {(oceny.Any() ? oceny.Average(x => x.OcenaKoncowa).ToString() : "Brak ocen")}"
            );
    }
